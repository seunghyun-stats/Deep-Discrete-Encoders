...
for aa = 1:5
    ...
    parfor(c= 1:C, n_parallel)
        rng(50+c);
        [X, A1_true, A2_true] = generate_X_normal(N, prop_true, B1, B2, gamma);
        
        tic;
        % spectral initialization
        [prop_ini(:,c), B1_ini(:,:,c), B2_ini(:,:,c), gamma_ini(:,c), A1_est, A2_est, err_G1_ini(:,c), err_G2_ini(:,c)] = Normal_init(X, K1, K1, K2, B1, B2, G1, G2, epsilon);
        
        % SAEM
        [prop_final(:,c), B1_final(:,:,c), B2_final(:,:,c), gamma_fin(:,c), A1_final, A2_final, itera(c)] = get_SAEM_normal( ...
             X, prop_ini(:,c)', B1_ini(:,:,c), B2_ini(:,:,c), gamma_ini(:,c), lambda_1, ...
             lambda_2, tau, A1_est, A2_est, 1, tol);
    
        % uncomment below to implement PEM instead of SAEM
    %     [prop_final(:,c), B1_final(:,:,c), B2_final(:,:,c), phi{c}, loglik_est(c)] = get_EM_Normal( ...
    ...
    for c = ind
        ...
        mse_theta(c) = (sum((prop_final(:,c)' - prop_true).^2) + sum((B1_est - B1).^2, 'all') + ...
            sum((B2_est - B2).^2, 'all') + sum(gamma_fin(:,c) - gamma).^2)/(K2+ K1*(K2+1) + J*(K1+1) + J);
    ...
    save("Gaussian_sid_SAEM_"+N+"_"+K2+'.mat')
end
