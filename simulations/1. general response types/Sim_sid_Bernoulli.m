...
for aa = 1:5
    ...
    parfor(c= 1:C, n_parallel)
        rng(50+c);
        [X, A1_true, A2_true] = generate_X_bernoulli(N, prop_true, B1, B2);
        
        tic;
        % spectral initialization
        [prop_ini(:,c), B1_ini(:,:,c), B2_ini(:,:,c), A1_est, A2_est, err_G1_ini(:,c), err_G2_ini(:,c)] = Ber_init(X, K1_max, K1, K2, G1, G2, epsilon);

        %% EM
        [prop_final(:,c), B1_final(:,:,c), B2_final(:,:,c), A1_final, A2_final, p, q, itera(c), ~] = get_SAEM_bernoulli( ...
             X, prop_ini(:,c)', B1_ini(:,:,c), B2_ini(:,:,c), lambda_1, ...
             lambda_2, tau, A1_est, A2_est, 1, tol);

        % uncomment below to implement PEM instead of SAEM
    %     [prop_final(:,c), B1_final(:,:,c), B2_final(:,:,c), phi{c}, loglik_est(c)] = get_EM_bernoulli( ...
    ...
    save("Bernoulli_sid_SAEM_"+N+"_"+K2+'.mat')
end
